...
// Code Editor search result types
...
export interface FileAPI {
  ...
  codeEditorSearchText: (workspaceRoot: string, query: string) => Promise<IPCResult<SearchResult[]>>;
}
...
export const createFileAPI = (): FileAPI => ({
  ...
  codeEditorSearchText: (workspaceRoot: string, query: string): Promise<IPCResult<SearchResult[]>> =>
    ipcRenderer.invoke(IPC_CHANNELS.CODE_EDITOR_SEARCH_TEXT, workspaceRoot, query)
});
